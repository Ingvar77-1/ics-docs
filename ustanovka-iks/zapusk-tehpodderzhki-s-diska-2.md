---
id: "article_358"
title: "Запуск техподдержки с диска"
description: "При возникновении проблем с загрузкой системы для настройки удаленного подключения можно воспользоваться установочным диском ИКС."
type: article
section: "ustanovka-iks"
parent_id: "category_9"
level: 2
order: 10
source_url: "https://doc.a-real.ru/index.php?article=358"
processed_at: "2026-07-14"
images_count: 5
tags: [икс, установка, техподдержка, загрузка, диск]
---

# Запуск техподдержки с диска

При возникновении проблем с загрузкой системы (например, в результате сбоя питания) для настройки удаленного подключения можно воспользоваться установочным диском ИКС.

---

При возникновении проблем с загрузкой системы (например, в результате сбоя питания) для настройки удаленного подключения можно воспользоваться установочным диском ИКС.

Для этого выполните следующие действия:

1. Запустите загрузку с диска.
2. Во время запуска установщика выберите **язык**.
3. Нажмите **«Support»**.
4. Укажите следующие **параметры сетевого адаптера ИКС**:
   - сетевой интерфейс, к которому будет подключена внутренняя локальная сеть;

     ![Иллюстрация](../images/article_358/support2.png)

   - [IP-адрес](https://doc.a-real.ru/index.php?article=24#ip-address);

     ![Иллюстрация](../images/article_358/support3.png)

   - маска подсети;

     ![Иллюстрация](../images/article_358/support4.png)

   - шлюз по умолчанию.

     ![Иллюстрация](../images/article_358/support5.png)
5. После введения параметров запустится служба технической поддержки и система укажет **порт подключения**. Сообщите его специалисту службы технической поддержки ([наши контакты](https://xserver.a-real.ru/#main-footer)).

   ![Иллюстрация](../images/article_358/support6.png)

> ⚠️ Важно! Если вы используете PPPoE провайдера, то вам требуется подключить маршрутизатор для получения его настроек.

---

**Источник:** [Документация ИКС — Запуск техподдержки с диска](https://doc.a-real.ru/index.php?article=358)
