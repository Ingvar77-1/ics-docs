...
Поле **«Порт HTTP»** предназначено для указания порта, по которому веб-сервер принимает [HTTP](https://doc.a-real.ru/index.php?article=24#http)-запросы. По умолчанию это порт 80.

Поле **«Порт HTTPS»** предназначено для указания порта, по которому веб-сервер принимает [HTTPS](https://doc.a-real.ru/index.php?article=24#https)-запросы. По умолчанию это порт 443.

В поле **«Сертификат для HTTPS»** можно назначить службе заранее созданный [сертификат](https://doc.a-real.ru/index.php?article=78) для работы сервера по защищенному протоколу HTTPS с использованием [SSL](https://doc.a-real.ru/index.php?article=24#ssl).

При установке флага **«Перенаправлять с HTTP на HTTPS»** веб-сервер всегда будет работать по защищенному соединению.

Флаг **«Автоматически создавать разрешающее правило»** создает [разрешающее правило](https://doc.a-real.ru/index.php?article=206) межсетевого экрана на HTTP/HTTPS-порты веб-сервера из внешних сетей.
...
- **через домен** — позволяет авторизоваться на ресурсах веб-сервера пользователям, импортированным в ИКС из домена. Авторизация возможна, если в [настройках](https://doc.a-real.ru/index.php?article=190#tab2) [LDAP](https://doc.a-real.ru/index.php?article=190#tab2) [-авторизации](https://doc.a-real.ru/index.php?article=190#tab2) указаны корректные данные для подключения к LDAP-серверу. Для авторизации через LDAPS необходимо импортировать в ИКС корневой сертификат центра сертификации, выдавшего сертификат для LDAP-сервера. После импорта сертификата установите в нем флаг **«Добавить в доверенные сертификаты»**, а затем укажите данный сертификат в настройках LDAP-авторизации;
...
- [Веб-ресурс](https://doc.a-real.ru/index.php?article=260)
- [Виртуальный хост](https://doc.a-real.ru/index.php?article=261)
- [Виртуальный хост с перенаправлением](https://doc.a-real.ru/index.php?article=262)
...
**Источник:** [Документация ИКС — Веб](https://doc.a-real.ru/index.php?article=81)
