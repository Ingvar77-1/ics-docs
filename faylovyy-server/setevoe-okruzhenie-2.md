...
Для обмена данными в локальной сети используется протокол [SMB](https://doc.a-real.ru/index.php?article=24#smb). В ИКС за реализацию этого протокола отвечает служба «Сетевое окружение».
...
1. Введите **имя компьютера** и **описание**. Поле **«Имя компьютера»** отвечает за назначение сетевого [NetBIOS-имени](https://doc.a-real.ru/index.php?article=24#netbios_name) для сервера.
...
- находиться в **рабочей группе** — в сети предприятия не используется [контроллер домена](https://doc.a-real.ru/index.php?article=24#domain_controller) ([AD](https://doc.a-real.ru/index.php?article=24#ad)), компьютеры находятся в одной рабочей группе, [WINS](https://doc.a-real.ru/index.php?article=24#wins)-сервер отсутствует. По умолчанию ИКС находится в рабочей группе WORKGROUP, при необходимости ее можно переименовать.

- быть присоединенным к **домену** — в сети предприятия используется контроллер домена (AD). ИКС может быть присоединен к домену, что позволит [импортировать](https://doc.a-real.ru/index.php?article=131#ldap_ad) доменных пользователей, [синхронизировать](https://doc.a-real.ru/index.php?article=53) их, а также использовать [доменную авторизацию](https://doc.a-real.ru/index.php?article=190) на сетевых ресурсах ИКС.

  Для ввода ИКС в домен укажите **имя домена** и **[DNS](https://doc.a-real.ru/index.php?article=24#dns)-имя домена**. Установите флаг **«Создавать перенаправление DNS-зоны»** и укажите [DNS-сервер](https://doc.a-real.ru/index.php?article=24#dns-server) для перенаправления. Это может потребоваться, так как зачастую DNS-зона, в которой находится домен, не может быть разрешена внешними DNS-серверами.

4. При необходимости установите флаг **«Принудительно подписывать SMB запросы»**, который отвечает за подписывание с помощью [SMB](https://doc.a-real.ru/index.php?article=24#smb). Подписывание SMB, или подписи безопасности — это механизм обеспечения безопасности протокола SMB. Может не поддерживаться старыми операционными системами (Win98, WinXP).

5. Если требуется, установите флаг **«Разрешить SMBv1»**, который включает поддержку устаревших протоколов для взаимодействия с WindowsXP, [PHP](https://doc.a-real.ru/index.php?article=24#php)-скриптов и т. д.
...
**Источник:** [Документация ИКС — Сетевое окружение](https://doc.a-real.ru/index.php?article=83)
