...
[DNS](https://doc.a-real.ru/index.php?article=24#dns)-записи домена представляют собой записи в системе доменных имен о соответствии имени и служебной информации о сервере, на которое это имя должно указывать. Каждая такая запись необходима для работы определенной службы. Например, в DNS-запись типа MX вносятся данные для корректной работы электронной почты на домене.

Добавить запись [DNS-зоны](https://doc.a-real.ru/index.php?article=24#dns-zone) можно в [меню](https://doc.a-real.ru/index.php?article=60) **Сеть > DNS > Зоны**. Нажмите **«Добавить > Запись»** и выберите нужный объект.
...
Запись A (address record), или запись адреса, связывает имя хоста с [IP-адресом](https://doc.a-real.ru/index.php?article=24#ip-address).
...
Запись SRV (server selection) указывает на серверы для сервисов. Используется, в частности, для [Jabber](https://doc.a-real.ru/index.php?article=95) и [Active Directory](https://doc.a-real.ru/index.php?article=24#ad).

При создании записи можно указать следующие **параметры**:

- сервис — имя сервиса согласно RFC-3232 (IANA Assigned Port Numbers);
- протокол — протокол, по которому предоставляется сервис ([TCP](https://doc.a-real.ru/index.php?article=24#tcp), [UDP](https://doc.a-real.ru/index.php?article=24#udp));
...
Чтобы уменьшить объем нежелательной корреспонденции (спама), многие серверы-получатели электронной почты могут проверять наличие PTR-записи для хоста, с которого происходит отправка. В этом случае PTR-запись для IP-адреса должна соответствовать имени отправляющего почтового сервера, которым он представляется в процессе [SMTP](https://doc.a-real.ru/index.php?article=24#smtp)-сессии.

Данная запись применяется при создании [обратной DNS-зоны](https://doc.a-real.ru/index.php?article=227).
...
**Источник:** [Документация ИКС — Записи DNS-зоны](https://doc.a-real.ru/index.php?article=229)
