...
Добавить [провайдер](https://doc.a-real.ru/index.php?article=24#provider) можно в меню **Сеть > Провайдеры и сети**. Для этого выполните следующие действия:
...
4. Установите **переключатель**:
   ...
   - «Использовать следующий [IP-адрес](https://doc.a-real.ru/index.php?article=24#ip-address)» — укажите **диапазон адресов** в виде IP-адрес/префикс либо адрес:маска. Также требуется ввести адрес основного шлюза;
   - «Получить IP-адрес автоматически» — если провайдер выдает адреса по протоколу [DHCP](https://doc.a-real.ru/index.php?article=24#dhcp). При необходимости можно установить флаг **«Добавлять DNS провайдера в список DNS-серверов»**.
...
9. Если требуется, установите **флаги**:
   ...
   - «Разрешить управление ИКС через [SSH](https://doc.a-real.ru/index.php?article=24#ssh)» — будет разрешаться трафик от любого источника, идущий на IP-адрес провайдера на порт 22 через сетевой интерфейс, на котором настроен провайдер.

10. На вкладке можно задать [MAC-адрес](https://doc.a-real.ru/index.php?article=24#mac-address) интерфейса, а также **скорость**, **режим работы** и [MTU](https://doc.a-real.ru/index.php?article=24#mtu).
...
3. При помощи переключателя укажите тип выбора выдаваемого адреса:
   ...
   - автоматическое получение (в том случае, если ИКС находится за [NAT](https://doc.a-real.ru/index.php?article=24#nat)).
4. Нажмите кнопку **«Сохранить»**.

### Правила

Данная вкладка позволяет назначить правила [межсетевого экрана](https://doc.a-real.ru/index.php?article=27) для всего трафика, который проходит через данного провайдера.
...
**Источник:** [Документация ИКС — Провайдер](https://doc.a-real.ru/index.php?article=201)
