---
id: "article_213"
title: "Провайдер L2TP поверх IP/DHCP"
description: "Добавление провайдера L2TP поверх IP/DHCP в меню Сеть > Провайдеры и сети. Настройка вкладок: настройки IP, L2TP, шифрования и мониторинга."
type: "article"
section: "set/provaydery-i-seti"
parent_id: "category_44"
level: "3"
order: "16"
source_url: "https://doc.a-real.ru/index.php?article=213"
processed_at: "2026-07-14"
images_count: "5"
tags: "[икс, l2tp, провайдер, сеть, vpn]"
---

В большинстве случаев L2TP-соединение настраивается поверх текущего IP-протокола, поэтому создание L2TP-провайдера можно упростить путем совмещения настройки PPP- и IP-параметров.

---

Добавить провайдер L2TP поверх IP/DHCP можно в меню **Сеть &gt; Провайдеры и сети**. Для этого выполните следующие действия:

1. Нажмите кнопку **«Добавить»** и выберите **«Провайдеры &gt; Провайдер L2TP поверх IP/DHCP»**.

   ![Иллюстрация](../../images/article_213/52047-add_provider-13.0.png)

2. Заполните вкладку **«Настройки IP»** по аналогии с общими настройками [статического провайдера](provayder-2.md).

   ![Иллюстрация](../../images/article_213/52047-l2tp_ip1-13.0.png)

3. Введите данные на вкладке **«Настройки L2TP»** по аналогии с общими настройками [провайдера PPTP](provayder-pptp.md).

   ![Иллюстрация](../../images/article_213/52047-l2tp_ip2-13.0.png)

4. Определите параметры шифрования на вкладке **«Настройки шифрования»** по аналогии с настройкой [провайдера L2TP](provayder-l2tp-3.md).

   ![Иллюстрация](../../images/article_213/52047-l2tp_ip3-13.0.png)

5. Заполните вкладку **«Настройки мониторинга»** по аналогии с настройкой [статического провайдера](provayder-2.md).

   ![Иллюстрация](../../images/article_213/52047-l2tp_ip4-13.0.png)

6. Нажмите **«Добавить»** — новый провайдер появится в списке.

7. Для более детальных настроек провайдера откройте его [индивидуальный модуль](provayder-2.md).
