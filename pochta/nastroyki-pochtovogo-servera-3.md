...
description: "Настройка параметров работы почтового сервера ИКС: сетевые настройки, шифрование, отправка писем, списки ограничений и DKIM-подпись."
type: article
section: "pochta"
...
images_count: 9
tags: [икс, почта, почтовый сервер, SMTP, IMAP, POP3, шифрование, DKIM]
---
...
![Настройки почтового сервера](../../images/article_85/51224mail_settings1-13.2.png)
...
В соответствующих полях можно изменить стандартные порты приема и отправки почтовых сообщений ([SMTP](https://doc.a-real.ru/index.php?article=24#smtp), [POP3](https://doc.a-real.ru/index.php?article=24#pop3), [IMAP](https://doc.a-real.ru/index.php?article=24#imap)).
...
Флаг **«Включить SMTP через 587 порт»** позволяет почтовым клиентам (по умолчанию Mozilla Thunderbird) подключаться к серверу почты ИКС по 587/TCР порту вместо 25.

> ⚠ Внимание! Флаг не заменяет настройку «Порт SMTP», а дополняет ее.

При установке флага **«Автоматически создавать разрешающее правило»** в межсетевом экране будет создано [разрешающее правило](https://doc.a-real.ru/index.php?article=206) для доступа извне на порты SMTP, POP3 и IMAP. Перейти к списку существующих правил и их настройке можно нажатием на появившуюся гиперссылку «Доступ к почтовому серверу».
...
![Настройки почтового сервера — сетевые настройки](../../images/article_85/51224mail_settings1-13.2.png)
...
- Режим **«Обязательное»**. При авторизации пользоваться по протоколам SMTP, POP3/IMAP, [STARTTLS](https://doc.a-real.ru/index.php?article=24#starttls) пароль передается только внутри шифрованного соединения.

![Настройки шифрования почтового сервера](../../images/article_85/51224mail_settings2-13.2.png)

При выборе режимов «Необязательное» либо «Обязательное» установите следующие параметры:

- **«Сертификат для SMTP»** — позволяет выбрать [сертификат](https://doc.a-real.ru/index.php?article=78) для протокола SMTP из заведенных на ИКС. Включает использование шифрованного соединения по методу STARTTLS поверх использования обычного [TCP](https://doc.a-real.ru/index.php?article=24#tcp)-соединения по протоколу SMTP на стандартном порту 25. Данное шифрование является компромиссным. Если удаленная сторона не поддерживает шифрование, то письмо будет отправляться (приниматься) по нешифрованному протоколу SMTP.

- **«Сертификат для IMAP/POP3»** — позволяет выбрать [сертификат](https://doc.a-real.ru/index.php?article=78) для протоколов IMAP и POP3 из заведенных на ИКС. Включает использование шифрованного соединения по методу STARTTLS поверх использования обычного TCP-соединения по протоколам IMAP и POP3 на стандартных портах 143 и 110 соответственно.
...
- **«Длина ключа DH (Diffie-Hellman)»** — позволяет установить длину ключа при шифровании методом STARTTLS и для криптографических протоколов [TLS](https://doc.a-real.ru/index.php?article=24#tls) различных версий при использовании протоколов IMAP/POP3 и IMAPS/POP3S. Рекомендуемая длинна ключа 2048 бит. По умолчанию установлена длина 1024 бита, это требуется для оптимизации первого запуска ИКС.

- Флаг **«Автоматически создавать разрешающее правило»** — позволяет автоматически создавать в межсетевом экране [разрешающее правило](https://doc.a-real.ru/index.php?article=206).

### Отправка писем {#send}

Для задания различных ограничений при отправке писем можно использовать следующие настройки:

- **«Максимальный размер письма»** — задает ограничение на загрузку вложений через [веб-почту](https://doc.a-real.ru/index.php?article=86) (встроенный клиент Roundcube). Значение устанавливается в мегабайтах;

- флаг **«Ограничить частую отправку писем»** — включает ограничения на отправку писем через почтовый сервер ИКС;

- **«Максимальное количество писем с одного IP-адреса в минуту»** — задает величину максимального количества писем, отправляемых за одну минуту, с одного [IP-адреса](https://doc.a-real.ru/index.php?article=24#ip-address). Данное ограничение не действует на письма, которые отправлены из веб-интерфейса предустановленного клиента электронной почты;
...
![Настройки отправки писем](../../images/article_85/51224mail_settings3-13.2.png)
...
- **«Интервал между попытками отправки»** — позволяет задать время, по истечении которого после отправки письма [демон](https://doc.a-real.ru/index.php?article=24#daemon) будет проверять срок нахождения письма в очереди (в минутах). По умолчанию установлено 30 минут.
...
Чтобы включить отправку исходящей почты через другой SMTP-сервер, введите его адрес (доменное имя или IP) в поле **«[Релей](https://doc.a-real.ru/index.php?article=24#relay) по умолчанию»** и задайте порт для подключения.
...
![Отправка через внешний SMTP](../../images/article_85/51224mail_settings4-13.2.png)
...
С указанных адресов ИКС будет всегда принимать почтовые сообщения без проверки серыми списками и без проверки соответствия прямой и обратной записей в [DNS](https://doc.a-real.ru/index.php?article=24#dns), а также без авторизации.
...
![Общие настройки почтового сервера](../../images/article_85/51224mail_settings5-13.2.png)

> **«Жесткий диск для хранения почты»** — позволяет переместить хранилище почты на отдельный [жесткий диск](https://doc.a-real.ru/index.php?article=111). По умолчанию почта хранится в основном системном разделе (там, где установлен ИКС). При изменении места хранения почты будет произведено копирование всех писем с текущего жесткого диска на новый. Ход копирования почты с диска на диск можно отслеживать в [меню](https://doc.a-real.ru/index.php?article=107) **Обслуживание &gt; Система &gt; Задачи**. Если новый жесткий диск уже содержит файлы с почтой, копирование производиться не будет.
...
Блок включает проверку входящих и исходящих писем на наличие в них вирусов. При положительном результате вместо письма получателю придет сообщение о результатах проверки, а само письмо будет во вложении к сообщению. Чтобы активировать проверку [антивирусом ClamAV](https://doc.a-real.ru/index.php?article=66) либо [антивирусом Касперского](https://doc.a-real.ru/index.php?article=68), установите соответствующий флаг.

### DKIM-подпись

В данном блоке выполняются следующие настройки:

- флаг **«Проверять DKIM-подпись»** — включает проверку входящих писем на наличие и правильность [DKIM-подписи](https://doc.a-real.ru/index.php?article=24#dkim-sign);
...
- **«Селектор»** — позволяет для каждого почтового сервера в одном домене создавать свой [DKIM](https://doc.a-real.ru/index.php?article=24#dkim)-селектор. Это нужно потому, что для одного домена может быть несколько почтовых серверов. По умолчанию в ИКС используется селектор default.

![Настройки DKIM-подписи](../../images/article_85/51224mail_settings6-13.2.png)
...
![Защита от спама](../../images/article_85/51224mail_settings7-13.2.png)

### Черные списки DNSBL

Блок позволяет добавлять и удалять хосты, содержащие [черные списки DNSBL](https://doc.a-real.ru/index.php?article=24#blacklist). Данные списки используются для борьбы со спамом.
...
При установке флага **«Использовать [серые списки](https://doc.a-real.ru/index.php?article=24#greylisting)»** ИКС будет отслеживать поведение почтовых серверов, которые отправляют письма на ИКС. О методологии блокировки можно прочитать [здесь](https://ru.wikipedia.org/wiki/%D0%A1%D0%B5%D1%80%D1%8B%D0%B9_%D1%81%D0%BF%D0%B8%D1%81%D0%BE%D0%BA).
...
- флаг **«Использовать Адресную книгу ИКС»** — позволяет включить или выключить использование адресной книги всеми почтовыми клиентами. Передача адресной книги в Roundcube настраивается в [веб-почте](https://doc.a-real.ru/index.php?article=86);

- **«Base DN»** — позволяет настроить параметр Base DN (базу поиска для [LDAP](https://doc.a-real.ru/index.php?article=24#ldap)), можно указать несколько через точку с запятой;
...
![Адресная книга почтового сервера](../../images/article_85/51224mail_settings8-13.2.png)
...
![Экспорт адресной книги](../../images/article_85/51224mail_settings9-13.2.png)
...
**Источник:** [Документация ИКС — Настройки почтового сервера](https://doc.a-real.ru/index.php?article=85)
