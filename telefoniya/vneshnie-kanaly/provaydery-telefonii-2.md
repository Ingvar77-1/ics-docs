...
Данный провайдер предназначен для установки соединения с использованием протокола [SIP](https://doc.a-real.ru/index.php?article=24#sip).
...
9. Поля **«Режим DTMF»**, **«Опция insecure»**, **«Опция canreinvite»** позволяют настроить режимы тонального набора, при этом два последних поля доступны только при выборе драйвера chan_sip в [настройках телефонии](https://doc.a-real.ru/index.php?article=100#block4). Поле **«Опция direct_media»** доступно только для драйвера chan_pjsip и определяет, могут ли медиаданные передаваться напрямую между конечными точками. Если нет (значение no), то все RTP-потоки проходят через [Asterisk](https://doc.a-real.ru/index.php?article=24#asterisk).
...
10. В поле **«Тип подключения»** можно выбрать, использовать ли шифрование SIP-пакетов и медиаданных (RTP) для выбранного номера с помощью сертификата, который установлен при [настройке сервера телефонии](https://doc.a-real.ru/index.php?article=100#block3) в поле **«Сертификат для шифрования (TLS и SRTP)»**. Возможны следующие варианты подключения:
    ...
    Каждый тип подключения требует соответствующей установки своего транспорта, которая задаётся в [настройках сервера телефонии](https://doc.a-real.ru/index.php?article=100).
...
Данный провайдер предназначен для установки соединения с использованием протокола [IAX](https://doc.a-real.ru/index.php?article=24#iax).
...
3. Заполните поля открывшегося окна по аналогии с [провайдером SIP](https://doc.a-real.ru/index.php?article=247#sip).
...
**Источник:** [Документация ИКС — Провайдеры телефонии](https://doc.a-real.ru/index.php?article=247)
