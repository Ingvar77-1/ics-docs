...
В блоке можно изменить настройки портов [SIP](https://doc.a-real.ru/index.php?article=24#sip) (через [UDP](https://doc.a-real.ru/index.php?article=24#udp)) и [IAX](https://doc.a-real.ru/index.php?article=24#iax), а также диапазон портов для входящих соединений [RTP](https://doc.a-real.ru/index.php?article=24#rtp). По умолчанию используются следующие значения:
...
При установке флага **«Автоматически создавать разрешающее правило»** в межсетевом экране добавится [разрешающее правило](https://doc.a-real.ru/index.php?article=206) для указанных портов. Перейти к правилу можно нажатием на ссылку в его названии.
...
В поле **«Сертификат для шифрования (TLS и SRTP)»** можно выбрать, изменить или удалить [сертификат](https://doc.a-real.ru/index.php?article=78) шифрования для телефонии. По умолчанию выбран автоматически сгенерированный конечный сертификат (самоподписанный).
...
> ⚠️ Внимание! Чтобы использовать шифрование, в настройках конкретного [телефонного номера](https://doc.a-real.ru/index.php?article=102) необходимо выбрать тип подключения **«С шифрованием (TLS и SDES sRTP)»**, а также настроить шифрование на стороне абонента.
>
...
Chan_pjsip — это более новый канальный драйвер канала SIP в [Asterisk](https://doc.a-real.ru/index.php?article=24#asterisk). При переключении на данный драйвер часть настроек IP-телефонии поменяется.

![Драйвер канала SIP](../../images/article_100/tel_settings3.png)

Отличия в настройке [внешних каналов](https://doc.a-real.ru/index.php?article=103):
...
Флаг **«SRV lookup»** активирует [DNS](https://doc.a-real.ru/index.php?article=24#dns)-поиск [SRV-записей](https://doc.a-real.ru/index.php?article=24#srv). Данный флаг недоступен при выборе chan_pjsip в качестве драйвера канала SIP.
...
> ⚠️ Флаг **«Отправлять сообщения голосовой почты на e-mail»** — глобальная настройка данной опции. Если флаг не установлен, то опция «Отправлять сообщения голосовой почты на почту» в настройках [телефонного номера](https://doc.a-real.ru/index.php?article=242) работать не будет. При этом для конкретного телефонного номера можно отдельно отключить отправку на e-mail. Если же оставить данную настройку включенной только у внутренних номеров, то не будет возможности отключить ее у всех номеров сразу.
>
...
Данный блок отвечает за настройку поведения модуля телефонии, если он находится за [NAT](https://doc.a-real.ru/index.php?article=24#nat).

Флаг **«Использовать NAT (ICS за NAT)»** включает преобразование [IP-адресов](https://doc.a-real.ru/index.php?article=24#ip-address) внутри пакетов телефонии.
...
- внешний [SMTP](https://doc.a-real.ru/index.php?article=24#smtp)-сервер — заполните поля **«SMTP-сервер»**, **«Порт»**, **«Логин»**, **«Пароль»** и **«Адрес отправителя»**, а также при необходимости установите флаг **«SSL»**.

## Кодеки

В данном блоке можно выбрать заданные по умолчанию [кодеки](https://doc.a-real.ru/index.php?article=24#codec), которые будут использоваться модулем телефонии для всех номеров.
...
**Источник:** [Документация ИКС — Настройки сервера телефонии](https://doc.a-real.ru/index.php?article=100)
