...
# Индивидуальный модуль пользователя (группы)
...
- [IP-адреса](https://doc.a-real.ru/index.php?article=24/#ip-address), присвоенные пользователю;
...
- **«Выключить»** — выключить пользователя: на 5 минут, 30 минут, 1 час, 1 день или задать дату и время, до которого будет выключен пользователь. Трафик от выключенного пользователя будет блокироваться [межсетевым экраном](https://doc.a-real.ru/index.php?article=24#firewall), и данный пользователь не сможет выйти в сеть Интернет;
...
Редактирование, выключение или удаление синхронизированных пользователей из [LDAP](https://doc.a-real.ru/index.php?article=24/#ldap) невозможно на стороне ИКС. Все изменения вносятся со стороны сервера AD.
...
Содержит список IP/[MAC-адресов](https://doc.a-real.ru/index.php?article=24/#mac-address), закрепленных за данным пользователем. Эти адреса могут использоваться для [авторизации пользователя](https://doc.a-real.ru/index.php?article=137) на ИКС. Одному пользователю можно назначить несколько IP-адресов и несколько MAC-адресов.
...
Более подробно управление IP/MAC-адресами пользователя описано [здесь](https://doc.a-real.ru/index.php?article=137).

![Иллюстрация](../../images/article_142/individ3.png)

## Правила и ограничения

На вкладке можно управлять [правилами доступа](https://doc.a-real.ru/index.php?article=146) пользователя:
...
Если у пользователя создан телефонный номер и в ИКС заданы [перенаправления](https://doc.a-real.ru/index.php?article=104#tab2), в которых этот номер указан в поле **«При неответе номера»**, на вкладке будут отображены все такие перенаправления. Те перенаправления, в которых этот номер указан как единственный, будут доступны для редактирования.

![Иллюстрация](../../images/article_142/individ6.png)

## OpenVPN

На вкладке показано, доступно ли пользователю использование [OpenVPN-соединения](https://doc.a-real.ru/index.php?article=63).

Если для пользователя было добавлено разрешение на использование [OpenVPN](https://doc.a-real.ru/index.php?article=24/#openvpn)-соединения, станут доступны следующие параметры настройки:
...
- поле **«Передать клиентам маршруты до сетей»** — позволяет передать пользователю информацию об указанных [LAN](https://doc.a-real.ru/index.php?article=24/#lan);
...
- Сеть типа **«OpenVPN DCO»**. В режиме OpenVPN DCO статический маршрут в веб-интерфейсе не создается автоматически, так как маршрутизация осуществляется средствами ядра и для создания маршрута необходимо знать адрес пользователя в этой сети. В связи с этим для организации доступа к сетям, расположенным за клиентом, предусмотрены два способа:
  - Автоматическое создание временного маршрута. Маршруты до сетей, указанных в поле «Удаленные сети за клиентом», создаются автоматически после подключения клиента. Эти маршруты не отображаются [на вкладке](https://doc.a-real.ru/index.php?article=58#static) **«Сеть > Маршруты > Статические маршруты»**. Просмотреть их можно [на вкладке](https://doc.a-real.ru/index.php?article=57#routing_tables) **«Сеть > Сетевые утилиты > Таблица маршрутизации»**.
  - Ручное добавление постоянного статического маршрута. За пользователем закрепляется IP-адрес на вкладке **«OpenVPN»** [индивидуального модуля пользователя](https://doc.a-real.ru/index.php?article=142#tab7). После закрепления IP-адреса необходимо вручную добавить статический маршрут через данный адрес.
...
Если пользователь авторизуется с помощью утилиты [Xauth](https://doc.a-real.ru/index.php?article=187), на данной вкладке показан список приложений, в которых работает пользователь. Отображаются следующие данные:
...
Если пользователь не авторизован на [сервере авторизации (Xauth)](https://doc.a-real.ru/index.php?article=51), на вкладке отобразится соответствующее сообщение.

![Иллюстрация](../../images/article_142/individ9.png)

## Статистика

Вкладка является [конструктором отчетов](https://doc.a-real.ru/index.php?article=144) и предназначена для формирования отчета по пользователю.
...
**Источник:** [Документация ИКС — Индивидуальный модуль пользователя (группы)](https://doc.a-real.ru/index.php?article=142)
