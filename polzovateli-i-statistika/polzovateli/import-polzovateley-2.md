...
2. Выберите **LDAP-сервер**: Active Directory либо FreeIPA.
   ...
   > ⚠ Внимание! Начиная с версии 8.4, пользователи, импортированные из FreeIPA, могут авторизоваться в веб-интерфейсе, на прокси-сервере (с использованием [Kerberos](../nastroyki-avtorizacii-2.md)) и почтовом сервере. Подключение по VPN таких пользователей на данный момент недоступно. Для импорта пользователей необходимо указать логин пользователя как DN пользователя в LDAP-каталоге (например, `uid=admin,cn=users,cn=accounts,dc=ipa,dc=test`).
...
Группы можно изменить в [константах](../../konstanty/konstanty-obzor-3.md).
...
Импортированные почтовые ящики также будут иметь иконку ![icon-synchron.png](../../images/article_131/icon-synchron.png) и отобразятся в [меню](../../pochta/domeny-i-yaschiki-2.md) **Почта > Домены и ящики**.

Для более детальных настроек синхронизации пользователей воспользуйтесь [службой синхронизации](../sinhronizaciya-2.md).
