...
Добавить **запрещающее правило прокси** можно на вкладке **«Правила и ограничения»** в [индивидуальном модуле пользователя (группы)](../polzovateli/individualnyy-modul-polzovatelya-gruppy-2.md), который расположен в меню **Пользователи и статистика &gt; Пользователи**.
...
3. В раскрывающихся списках можно выбрать:
   - [URL](../../o-dokumentacii/slovar-terminov-3.md) **назначения** — в качестве назначения возможно указывать: IP-адрес; IP/mask; имя домена (например, `ya.ru`); поддомены, исключая основной домен (например, `.google.com` — при обращении на `drive.google.com` авторизация не будет запрошена, но при обращении на `google.com` авторизация запрошена будет); регулярное выражение в формате `/regex/gi` (например, `/.*.ai.\.ru/gi` — разрешит домен `mail.ru` и его поддомены);
   - **протокол**;
   ...
   - [IP-адрес](../../o-dokumentacii/slovar-terminov-3.md) **пользователя**.
   ...
   Поэтому если сохранить запрещающее правило прокси по умолчанию (все поля со значением «любой») и применить его к пользователю (группе), **прокси-сервер запретит все коммуникации, идущие через него** (по протоколам [HTTP](../../o-dokumentacii/slovar-terminov-3.md), [HTTPS](../../o-dokumentacii/slovar-terminov-3.md), [FTP](../../o-dokumentacii/slovar-terminov-3.md) и HTTP/HTTPS). Если не был создан сертификат, запрещающее правило прокси не будет действовать на HTTPS-трафик.
...
> ⚠ Важно! Для корректной работы действий «Запретить доступ» и «Вывести сообщение» необходимо, чтобы метод CONNECT был разрешен. То есть при создании запрещающего правила прокси, если в поле «Метод» указано «любой», необходимо создать [разрешающее правило прокси](razreshayuschee-pravilo-proksi.md), в котором указать в поле «Метод» значение CONNECT. Также прокси-сервер должен работать в режиме «Расшифровка трафика с подменой сертификата».
